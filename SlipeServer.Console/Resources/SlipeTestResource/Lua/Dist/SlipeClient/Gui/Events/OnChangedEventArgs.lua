-- Generated by CSharp.lua Compiler
local System = System
local SlipeLuaSharedElements = SlipeLua.Shared.Elements
local SlipeLuaClientGui
System.import(function (out)
  SlipeLuaClientGui = SlipeLua.Client.Gui
end)
System.namespace("SlipeLua.Client.Gui.Events", function (namespace)
  namespace.class("OnChangedEventArgs", function (namespace)
    local getElement, __ctor__
    __ctor__ = function (this, element)
      setElement(this, SlipeLuaSharedElements.ElementManager.getInstance():GetElement(element, SlipeLuaClientGui.EditableGuiElement))
    end
    getElement = System.property("Element", true)
    return {
      getElement = getElement,
      __ctor__ = __ctor__,
      __metadata__ = function (out)
        return {
          properties = {
            { "Element", 0x206, out.SlipeLua.Client.Gui.EditableGuiElement, getElement }
          },
          methods = {
            { ".ctor", 0x104, nil, out.SlipeLua.MtaDefinitions.MtaElement }
          },
          class = { 0x6 }
        }
      end
    }
  end)
end)
